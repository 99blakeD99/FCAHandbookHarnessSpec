...
**Extracting from data:** In the Handbook, the rule type appears as a single letter in the `regulatory_content` field, immediately following the rule ID number:
...
**Computing from data:** In the Handbook, each record includes a `level` field indicating document depth:
...
## Current Weights (Authoritative YAML)

The definitive regulatory weights are specified in YAML format below. This is the source of truth for all implementations.

**Important:** Do not modify this section without updating the corresponding `weights.yaml` file in the implementation, and document the regulatory rationale in a git commit message.

```yaml
# FCA Handbook Regulatory Weighting Configuration
# AUTHORITATIVE: This YAML block is the definitive source
# Implementation: Load from weights.yaml in the harness (which mirrors this block)
...
hierarchy_multipliers:
  1: 1.2              # Top-level section (e.g., "COBS 2")
  2: 1.0              # Subsection (e.g., "COBS 2.1")
  3: 0.9              # Nested items, guidance, sub-guidance (3+)
...
The `semantic_search` action (WorkflowSpecPrinciples.md) implements structured search:

```
INPUT: Product features + Compliance question
  ↓
1. Embed question using the same embedding model as the Handbook
2. Load the Handbook (in-memory JSON)
